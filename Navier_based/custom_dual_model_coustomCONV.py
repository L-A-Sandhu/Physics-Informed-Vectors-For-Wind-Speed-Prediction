...
from conv_lstm2d import ConvLSTM2D  # Import your custom ConvLSTM2D class
...
class VolatilityAwareWindPredictor:
    ...
    def build_custom_model(self, label):
        ...
        x = ConvLSTM2D(filters=64, kernel_size=(3, 3), 
                       activation='tanh', return_sequences=True, padding="same", name=f'{label}_conv_lstm1')(x)

        x = ConvLSTM2D(filters=32, kernel_size=(3, 3), 
                       activation='tanh', return_sequences=True, padding="same", name=f'{label}_conv_lstm2')(x)
        ...
        # memberships_repeated = layers.Lambda(lambda x: tf.tile(x[:, tf.newaxis, :], [1, 24, 1]), name=f'{label}_membership_repeated')(memberships)
        # x = layers.Concatenate(axis=-1, name=f'{label}_concatenate')([x, memberships_repeated])
